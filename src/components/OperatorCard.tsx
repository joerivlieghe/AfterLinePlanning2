...
import { WrenchIcon, ClockIcon, GaugeIcon, TruckIcon, InfoIcon } from 'lucide-react';
  ...
  return (
    <Card
      className={`w-full bg-white shadow-lg rounded-lg overflow-hidden transform transition-transform hover:scale-[1.02] hover:shadow-xl cursor-pointer ${
        isSelected ? 'border-2 border-blue-500 ring-2 ring-blue-500' : '' // Apply border if selected
      }`}
      onClick={() => onClick && onClick(operator.id)}
    >
      <CardHeader className="p-4 pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg font-semibold text-gray-800">{operator.name}</CardTitle>
      ...
      <CardContent className="p-4 pt-0 space-y-2 text-gray-700">
        ...
        {operator.status === 'Busy' && operator.assignedTrucks.length > 0 && (
          <div className="mt-2 text-sm text-gray-600">
            <span className="font-semibold">Current Assignment:</span> {operator.assignedTrucks[0].model} ({operator.assignedTrucks[0].vin})
          </div>
        )}
      </CardContent>
    </Card>
